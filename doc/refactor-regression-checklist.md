...
**Migration complete (2026-06-24).** 8 tasks, one commit each. `app/page.jsx` is now a 10-line route file (was 5246 at branch start; ~2815 after the preserve-UI refactor); orchestration lives in `app/components/AppShell.jsx`. `npm run lint` 0 errors / 34 warnings (≤ baseline), `npm run build` prerenders `/`, `/market`, `/mine`. Desktop + mobile home/market/mine verified in-browser with 0 console errors (the only runtime errors observed were external Supabase `fund-valuation-ranking` connection failures from the market tab, unrelated to the refactor). Deferred follow-ups: deep memoization of the AppRuntime context objects (setters need useCallback first), and the deployed GitHub Pages repo-subpath deep-link check.
